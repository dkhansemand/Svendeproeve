<?php
    if(isset($POST['btnNewsEdit'])){
        $return = View::UseController()->EditNewsArticle($POST, $POST['_once_default'], Router::GetParamByName('ID'));
        if(isset($return['err']))
        {
        ...
        {
            $success = 'Nyheden er nu blevet rettet';
            unset($POST);
        }
    }
...
<section id="newsView">
    ...
    <form action="" method="post">
        ...
        <div class="input-field">
            ...
            <input type="text" name="newsTitle" id="newsTitle" placeholder="Nyheds titel" value="<?=$POST['newsTitle'] ?? $article->newsTitle ?? null?>" required> *
        </div>   
        <?php
        ...
        <div class="text-area">
            ...
            ?>
            <textarea name="newsContent" id="newsContent" cols="50" rows="10"><?=$POST['newsContent'] ?? html_entity_decode($article->newsContent) ?? null?></textarea>
        </div>
        <div class="input-field">
            <label for="newsStartDate">Start dato</label>
            <input type="date" name="newsStartDate" id="newsStartDate" value="<?=$POST['newsStartDate'] ?? $article->newsStartDate ?? null?>" required> *
            <?php
        ...
        <div class="input-field">
            ...
            <input type="date" name="newsEndDate" id="newsEndDate" value="<?=$POST['newsEndDate'] ?? $article->newsEndDate ?? null?>" required> *
            <?php
